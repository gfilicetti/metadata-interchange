...
"""
Test cases for GCPSampleAssetManager that make use of the OpenAssetIO
...
harness, by extending it with additional checks for GCPSampleAssetManagers
...
# GCPSampleAssetManager exposes an entry point, so can be pip installed without
...
class Test_GCPSampleAssetManager:
    def test_passes_apiComplianceSuite(self, harness_fixtures):
        assert harness.executeSuite(apiComplianceSuite, harness_fixtures)

    def test_passes_gcpsample_asset_manager_business_logic_suite(
        self, gcpsample_asset_manager_business_logic_suite, harness_fixtures
    ):
        assert harness.executeSuite(gcpsample_asset_manager_business_logic_suite, harness_fixtures)


class Test_GCPSampleAssetManager_Plugin:  # pylint: disable=too-few-public-methods
    def test_exposes_plugin_attribute_with_correct_type(self):
        import gcpsample_asset_manager  # pylint: disable=import-outside-toplevel

        assert issubclass(gcpsample_asset_manager.plugin, PythonPluginSystemManagerPlugin)


@pytest.fixture
def gcpsample_asset_manager_business_logic_suite(base_dir):
